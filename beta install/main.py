...
# --- Constants ---
VAULT_PATH = "_internals/vault.json"
#VAULT_PATH = "vault.json"
FIRSTBOOT = "_internals/firstboot.py"
...
if __name__ == '__main__':
    if os.path.exists(FIRSTBOOT):
        # Run firstboot in a completely separate process
        result = subprocess.run([sys.executable, FIRSTBOOT])
        if result.returncode != 0:
            print(f"firstboot exited with error code {result.returncode}")
